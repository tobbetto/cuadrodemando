<?php
/**
 * Version information for the Dashboard plugin
 *
 * @package    local_cuadrodemando
 * @author     Thorvaldur Konradsson
 * @version    1.0.0
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$plugin->version   = 2025080100;        // The current plugin version (Date: YYYYMMDDHH).
$plugin->requires  = 2020061500;        // Requires Moodle 3.10.1 or later.
$plugin->component = 'local_cuadrodemando'; // Full name of the plugin (used for diagnostics).
$plugin->maturity  = MATURITY_STABLE;   // This version's maturity level.
$plugin->release   = '1.1.0';           // Human-readable version name.
